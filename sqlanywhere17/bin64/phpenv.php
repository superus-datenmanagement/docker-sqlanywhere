...
$phpbasever = floatval( $phpver ) . ".0";

if ( !extension_loaded( 'sqlanywhere' ) ) {
    if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' ) {
        $load_result = dl( 'php-' . $phpver . '_sqlanywhere.dll' );
        if( !$load_result ) {
            $load_result = dl( 'php-' . $phpbasever . '_sqlanywhere.dll' );
        }
        if( !$load_result ) {
        ...
        if( !$load_result ) {
            $load_result = dl( 'php-' . $phpbasever . '_sqlanywhere_r.so' );
        }
        if( !$load_result ) {
            $load_result = dl( 'php-' . $phpver . '_sqlanywhere.so' );
        }
        if( !$load_result ) {
            $load_result = dl( 'php-' . $phpbasever . '_sqlanywhere.so' );
        }
        if( !$load_result ) {
